...
def clean_dataframe(df, columns_to_check=None, normalize_text=True):
    """
    Clean a pandas DataFrame by removing duplicate rows and optionally normalizing text columns.
    """
    cleaned_df = df.copy()
    
    # Remove duplicates based on specified columns or all columns
    if columns_to_check:
        cleaned_df = cleaned_df.drop_duplicates(subset=columns_to_check)
    else:
        cleaned_df = cleaned_df.drop_duplicates()
...
def validate_email_column(df, email_column):
    ...
    return df[email_column].str.match(pattern, na=False)