...
if __name__ == "__main__":
    ...
    removed_iqr = cleaner.remove_outliers_iqr(['feature_a'])
    print(f"Removed {removed_iqr} outliers using IQR method")
    
    cleaner.fill_missing(strategy='median', columns=['feature_b'])
    print("Missing values after filling:", cleaner.df.isna().sum().sum())
...
class DataCleaner:
    ...
    def normalize_zscore(self, columns=None):
        ...
        print(f"Normalized {len(columns)} columns using Z-score normalization")
        self.df = df_normalized
        return self
