...
def validate_data(data, required_columns, numeric_threshold=0.8):
    ...
    for col in numeric_cols:
        numeric_count = data[col].count()
        total_count = len(data)
        numeric_proportion = numeric_count / total_count if total_count > 0 else 0
        
        if numeric_proportion < numeric_threshold:
            validation_result['low_numeric_quality'].append({
                'column': col,
...
if __name__ == "__main__":
    ...
    validation = validate_data(cleaned, ['feature_a', 'feature_b', 'category'])
