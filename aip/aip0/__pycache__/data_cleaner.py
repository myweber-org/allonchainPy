...
def clean_dataset(df, drop_duplicates=True, fillna_strategy='drop'):
    """
    ...
    if fillna_strategy == 'drop':
        ...
        raise ValueError("Invalid fillna_strategy. Choose from: 'drop', 'fill_mean', 'fill_median', 'fill_mode'.")
...
if __name__ == "__main__":
    ...
    sample_df.loc[0, 'A'] = 1000
    sample_df.loc[1, 'B'] = 500
    
    print("Original DataFrame shape:", sample_df.shape)
    print("\nValidation results:")
    print(validate_dataframe(sample_df))
    
    cleaned_df = clean_numeric_data(sample_df)
...
def remove_outliers(df, column, threshold=3):
    ...
    return df.loc[valid_indices]import numpy as np
import pandas as pd
from scipy import stats
...
def clean_dataset(dataframe, config):
    ...
    if 'required_columns' in config:
        if not validate_dataframe(dataframe, config['required_columns']):
            raise ValueError("DataFrame missing required columns")
    
    if 'remove_duplicates' in config and config['remove_duplicates']:
        subset = config.get('duplicate_subset', None)
        dataframe = remove_duplicates(dataframe, subset=subset)
    
    if 'numeric_columns' in config:
        dataframe = clean_numeric_columns(dataframe, config['numeric_columns'])
