...
if __name__ == "__main__":
    remove_duplicates('raw_data.csv', 'cleaned_data.csv', subset=['id', 'email'])
import pandas as pd
import numpy as np
    ...
    if fill_missing and cleaned_df.isnull().sum().sum() > 0:
        for column in cleaned_df.select_dtypes(include=[np.number]).columns:
            if cleaned_df[column].isnull().sum() > 0:
                ...
                print(f"Filled missing values in column '{column}' with {missing_strategy}: {fill_value}")
...
def main():
    ...
    cleaned = clean_dataset(df, drop_duplicates=True, fill_missing=True, missing_strategy='mean')
