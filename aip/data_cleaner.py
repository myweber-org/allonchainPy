...
class DataCleaner:
    ...
    def standardize_columns(self, column_mapping: dict) -> 'DataCleaner':
        self.df = self.df.rename(columns=column_mapping)
        return self

    def fill_missing_values(self, strategy: str = 'mean', custom_value: Optional[float] = None) -> 'DataCleaner':
...
def create_sample_data() -> pd.DataFrame:
    data = {
        'ID': [1, 2, 3, 4, 5, 5, 6, 7],
        'Name': ['Alice', 'Bob', 'Charlie', 'David', 'Eve', 'Eve', 'Frank', 'Grace'],
        'Age': [25, 30, 35, None, 28, 28, 150, 32],
        'Salary': [50000, 60000, None, 70000, 80000, 80000, 90000, 100000]
    }
...
if __name__ == "__main__":
    ...
    result = (cleaner
              .remove_duplicates(subset=['ID', 'Name'])
              .standardize_columns({'Salary': 'Annual_Salary'})
              .fill_missing_values(strategy='mean')
              .remove_outliers('Age', method='iqr', threshold=1.5))
    
    cleaned_df = result.get_cleaned_data()
