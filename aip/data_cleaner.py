...
def process_dataset(file_path, column_name):
    """
    Complete pipeline to load, clean, and analyze dataset.
...
if __name__ == "__main__":
    ...
    stats_age = calculate_statistics(cleaned_data, 'age')
    print(f"Dataset cleaned. Statistics for age column: {stats_age}")
import pandas as pd
import numpy as np
from scipy import stats
...
def validate_data(data, check_missing=True, check_duplicates=True):
    ...
    validation_results['dtypes'] = data.dtypes.astype(str).to_dict()
...
if __name__ == "__main__":
    ...
    print("\nValidation results:")
    print(f"Rows: {validation['shape']['rows']}")
    print(f"Duplicates: {validation['duplicates']['count']}")
    print(f"Missing values in feature_a: {validation['missing_values']['counts']['feature_a']}")
import pandas as pd
import numpy as np
...
if __name__ == "__main__":
    sample_data = {
        'temperature': [22, 23, 24, 25, 26, 27, 28, 29, 100, 1],
