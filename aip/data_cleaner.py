...
if __name__ == "__main__":
    ...
    print("Missing values:", sample_data['value'].isnull().sum())
    
    cleaned_data = handle_missing_values(sample_data, strategy='mean')
    cleaned_data = remove_outliers_iqr(cleaned_data, 'value')
    cleaned_data = normalize_minmax(cleaned_data, 'score')
    cleaned_data = standardize_zscore(cleaned_data, 'value')
...
def example_usage():
    ...
    data = {
        ...
        'pressure': [1013, 1015, 1012, 1014, 1016, 500, 1013, 1015, 1012, 1014]
    }
    ...
    filtered_df = remove_outliers_iqr(cleaned_df, 'temperature')
    print("After removing temperature outliers:")
    print(filtered_df)
    print("\n")
    
    stats = calculate_basic_stats(filtered_df, 'temperature')
...
def clean_dataset(df, drop_na=True, column_case='lower'):
    ...
    Parameters:
    df (pd.DataFrame): Input DataFrame to clean.
    drop_na (bool): If True, drop rows with any null values. If False, fill nulls with column mean for numeric columns.
