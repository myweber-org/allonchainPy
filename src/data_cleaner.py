...
if __name__ == "__main__":
    ...
    validation_result = validate_dataset(cleaned_df, required_columns=['A', 'B', 'C', 'D'], min_rows=3)
    print(f"\nDataset validation result: {validation_result}")import pandas as pd
...
def handle_missing_values(df, column, strategy='mean'):
    ...
    if strategy == 'mean':
        ...
        return df_copy.dropna(subset=[column]).reset_index(drop=True)
    else:
        raise ValueError("Strategy must be 'mean', 'median', 'mode', or 'drop'")
...
def clean_dataset(df, drop_duplicates=True, fill_na=True, fill_value=0):
    ...
    fill_value: Value to fill nulls with (default: 0)
