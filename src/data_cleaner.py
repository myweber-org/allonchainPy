...
def clean_dataset(df, remove_duplicates=True, fill_missing=False, fill_value=0):
    ...
    fill_missing (bool): If True, fill missing values with specified fill_value.
    fill_value: Value to use for filling missing values when fill_missing is True.
...
def clean_data(df, numeric_columns):
    """Main function to clean data by removing outliers and normalizing."""
    if df is None or df.empty:
        print("DataFrame is empty or None. Cleaning aborted.")
        return df
    ...
    for col in numeric_columns:
        if col in df.columns:
            ...
            print(f"Column '{col}' not found. Skipping outlier removal.")
...
if __name__ == "__main__":
    sample_data = {
        'id': [1, 2, 2, 3, 4],
        'value': [10.5, np.nan, 15.0, np.nan, 20.0],
