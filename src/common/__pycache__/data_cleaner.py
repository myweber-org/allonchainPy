...
def clean_dataset(df, numeric_columns=None):
    ...
    Parameters:
    df (pd.DataFrame): Input DataFrame
    numeric_columns (list): List of numeric column names, defaults to all numeric columns
...
if __name__ == "__main__":
    ...
    sample_data['A'][:50] = np.random.uniform(500, 1000, 50)  # Add outliers
...
def example_usage():
    ...
    print(f"Summary statistics: {calculate_summary_statistics(df, 'values')}")
    
    cleaned_df = remove_outliers_iqr(df, 'values')
    print("\nCleaned DataFrame:")
    print(f"Shape: {cleaned_df.shape}")
    print(f"Summary statistics: {calculate_summary_statistics(cleaned_df, 'values')}")
