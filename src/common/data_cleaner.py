...
class DataCleaner:
    ...
    def handle_missing_values(self, strategy='mean', fill_value=None):
        if strategy == 'mean':
            ...
            raise ValueError("Invalid strategy or missing fill_value for custom strategy")
...
def clean_dataset(df, missing_strategy='mean', outlier_method='iqr'):
    ...
    for col in numeric_cols:
        if outlier_method == 'iqr':
            ...
            mask = z_scores > 3
            if mask.any():
                # Replace outliers with mean ± 3*std
                upper_limit = mean_val + 3 * std_val
                lower_limit = mean_val - 3 * std_val
                cleaned_df.loc[mask, col] = np.where(
                    cleaned_df.loc[mask, col] > mean_val, 
                    upper_limit, 
                    lower_limit
                )
...
if __name__ == "__main__":
    ...
    is_valid = validate_dataframe(cleaned_df, required_columns=['A', 'B', 'C'], min_rows=1)
...
if __name__ == "__main__":
    ...
    cleaned_data = clean_dataset(sample_data, [0, 1, 2])
