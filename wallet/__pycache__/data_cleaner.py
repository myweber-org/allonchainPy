...
def validate_dataframe(df, required_columns=None, min_rows=1):
    ...
    if not numeric_cols.empty:
        inf_count = (df[numeric_cols] == float('inf')).sum().sum() + (df[numeric_cols] == float('-inf')).sum().sum()
        if inf_count > 0:
...
def validate_dataframe(df, required_columns=None):
    ...
    for col in df.columns:
        validation_results['column_info'][col] = {
            ...
            'sample_values': df[col].dropna().iloc[:3].tolist() if len(df) > 0 else []
        }
...
class DataCleaner:
    ...
    def get_summary(self):
        summary = {
            ...
            'removed_rows': len(self.original_data) - len(self.data),
...
def clean_dataset(df, config):
    ...
    return cleaner.data, cleaner.get_summary()